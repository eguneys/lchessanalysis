...
export const piece_fen = (piece) => piece[0] === 'w' ? piece[1].toUpperCase() : piece[1];
...
export function make_right2(files, right) {
    let res = {};
    files.forEach(file => {
        let i = right[file];
        if (i) {
...
export function make_righter(files, right) {
    let res = {};
    files.forEach(file => {
        let _res = {};
        let i = file;
...
export function make_king_side(righter) {
    let res = {};
    poss.forEach(pos => {
...
export function make_forward(upper) {
    let res = {};
    poss.forEach(pos => {
        let [file, rank] = pos.split('');
        let _res = {};
...
function zip_pos(a, b) {
    return a.map((_a, i) => _a + b[i]);
}
export function make_fwd_que(upper, lefter) {
    let res = {};
    poss.forEach(pos => {
        let [file, rank] = pos.split('');
        let _res = {};
        ...
        Object.keys(_upper).map(_up => {
            let _left = Object.keys(_lefter).find(_left => _lefter[_left].length ===
                _upper[_up].length);
            if (_left) {
...
export function make_fwd2_que(up2, left) {
    let res = {};
    poss.forEach(pos => {
        let [file, rank] = pos.split('');
        poss.forEach(_pos => {
...
export function make_fwdn(forward, n) {
    let res = {};
    poss.forEach(pos => {
...
export const make_fwd2 = (forward) => make_fwdn(forward, 1);
export const make_fwd1 = (forward) => make_fwdn(forward, 0);
...
export const white_home = (pos) => pos.split('')[1] === '2';
export const black_home = (pos) => pos.split('')[1] === '7';
export const white_base_dif = (pos) => pos.split('')[1] !== '1';
export const black_base_dif = (pos) => pos.split('')[1] !== '8';
export function xy_xfilter(fwd2, white_home) {
    let res = {};
    poss.forEach(pos => {
